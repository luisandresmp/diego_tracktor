...
def detect_and_draw_contours(frame, thresh, meas_last, meas_now, min_area = 0, max_area = 10000):
    ...
    del meas_now[:]
    while i < len(l):
        area = cv2.contourArea(l[i])
        if area < min_area or area > max_area:
            del l[i]

        else:
            cv2.drawContours(final, l, i, (0,0,255), 1)
            M = cv2.moments(l[i])
            if M['m00'] != 0:
            	cx = M['m10']/M['m00']
            	cy = M['m01']/M['m00']
            ...
            i += 1
    return final, l, meas_last, meas_now
...
if __name__=='__main__':
    ...
    while(True):
        ...
        if ret == True:
            ...
            thresh = colour_to_thresh(frame, block_size, offset)
            ...
            final, meas_now, df = reorder_and_draw(final, colours, n_inds, col_ind, meas_now, df, mot, this)
